...
function cleanCompanyScope(values: AgentEditorValues): AgentEditorValues {
  const scope = values.knowledge_company_scope;
  return { ...values, knowledge_company_scope: scope ? {
    semantic_retrieval_enabled: scope.semantic_retrieval_enabled ?? false,
    default_company: scope.default_company?.trim() || null,
    companies: scope.companies.map((company) => ({ name: company.name.trim(),
      aliases: company.aliases.map((alias) => alias.trim()).filter(Boolean) })),
  } : null };
}
...
export default function AgentEditor({
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (mode === 'edit' && !guard.changed) {
      await onSubmit(cleanCompanyScope(form));
      return;
    }
    ...
    <form className="agent-editor" onSubmit={submit}>
      ...
      <section className="editor-section">
        ...
        <fieldset className="form-group">
          <legend>VAV realtime company scope</legend>
          <label>
            <input type="checkbox" checked={!!form.knowledge_company_scope}
              onChange={(event) => setForm({ ...form, knowledge_company_scope: event.target.checked
                ? { default_company: null, companies: [{ name: '', aliases: [] }] } : null })} />
            Use explicit company-scoped knowledge
          </label>
          <p className="form-hint">Use company names exactly as assigned to facts in the approved knowledge base. Aliases are caller wording, not additional knowledge access. Unattributed source text is excluded. This setting applies to VAV realtime, not externally hosted agents.</p>
          {form.knowledge_company_scope && <>
            <label>
              <input type="checkbox" checked={form.knowledge_company_scope.semantic_retrieval_enabled ?? false}
                onChange={(event) => setForm({ ...form, knowledge_company_scope: {
                  ...form.knowledge_company_scope!, semantic_retrieval_enabled: event.target.checked,
                } })} />
              Repair unmatched questions with AI
            </label>
            <p className="form-hint">Inworld single-pass: after an unmatched search, one bounded GPT-4o mini request reformulates the question. Uses the configured OpenAI account, adds token cost and up to a two-second model wait. Company scope stays locked; no answers are generated by this step.</p>
            {form.knowledge_company_scope.companies.map((company, index) => (
              <div className="form-grid" key={index}>
                <label>Company {index + 1}
                  <input required minLength={2} maxLength={160} value={company.name}
                    onChange={(event) => {
                      const scope = form.knowledge_company_scope!;
                      setForm({ ...form, knowledge_company_scope: {
                        ...scope,
                        default_company: scope.default_company === company.name ? event.target.value : scope.default_company,
                        companies: scope.companies.map((item, i) => i === index ? { ...item, name: event.target.value } : item),
                      } });
                    }} />
                </label>
                <label>Caller aliases (comma separated)
                  <input value={company.aliases.join(',')} placeholder="e.g. the group, head office"
                    onChange={(event) => {
                      const scope = form.knowledge_company_scope!;
                      setForm({ ...form, knowledge_company_scope: { ...scope, companies: scope.companies.map((item, i) =>
                        i === index ? { ...item, aliases: event.target.value.split(',') } : item) } });
                    }} />
                </label>
                <button type="button" disabled={form.knowledge_company_scope!.companies.length === 1}
                  onClick={() => {
                    const scope = form.knowledge_company_scope!;
                    setForm({ ...form, knowledge_company_scope: { ...scope,
                      default_company: scope.default_company === company.name ? null : scope.default_company,
                      companies: scope.companies.filter((_, i) => i !== index) } });
                  }}>Remove company {index + 1}</button>
              </div>
            ))}
            <button type="button" disabled={form.knowledge_company_scope.companies.length >= 50}
              onClick={() => setForm({ ...form, knowledge_company_scope: { ...form.knowledge_company_scope!,
                companies: [...form.knowledge_company_scope!.companies, { name: '', aliases: [] }] } })}>Add company</button>
            <label>Default company for a new call
              <select value={form.knowledge_company_scope.default_company ?? ''}
                onChange={(event) => setForm({ ...form, knowledge_company_scope: { ...form.knowledge_company_scope!, default_company: event.target.value || null } })}>
                <option value="">Ask the caller which company</option>
                {form.knowledge_company_scope.companies.filter((company) => company.name.trim()).map((company, index) =>

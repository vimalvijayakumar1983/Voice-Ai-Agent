                  ...
                  transcript.turns.length ? (
                    <div className="call-transcript">
                      {transcript.turns.filter((turn) => !['analysis', 'analysis_candidate'].includes(valueFromRecord(turn, ['role']))).map((turn, index) => {
                        ...
                        const sourceTool = valueFromRecord(turn, ['source_tool']);
                        const isCalculation = isSource && sourceTool === 'vav_compare_reports';
                        const isPreparedAnswer = isSource && ['vav_answer', 'vav_answer_clarification'].includes(sourceTool);
                        const sourceLabel = isCalculation ? 'VAV calculated comparison' : isPreparedAnswer ? 'VAV prepared answer' : 'MCP report';
                        const sourceDescription = isCalculation ? 'Calculated from MCP evidence — not an original ERP report' : isPreparedAnswer ? 'Prepared answer text — not an original MCP response' : 'Original MCP response — exact source';
                        const presentation = isSource ? valueFromRecord(turn, ['presentation_text']) : '';
                        return <div className="call-transcript-turn" key={`${index}-${speaker}`} lang={language || undefined} dir="auto"><div className={styles.transcriptMeta}><strong>{isSource ? sourceLabel : speaker}</strong><span>{isSource ? `Report delivery · ${valueFromRecord(turn, ['delivery_state']) || 'received'}` : language ? languageName(language) : 'Language not reported'}</span></div>{presentation ? <><strong>Prepared spoken answer</strong><p>{presentation}</p><small>This is the prepared presentation, not a recording transcript. Playback may have been interrupted.</small></> : null}{isSource ? <details open={!presentation}><summary>{sourceDescription}</summary><pre style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', font: 'inherit' }}>{content}</pre></details> : <p>{content}</p>}{isSource ? <small>Original reports, calculated comparisons and prepared answers are labelled separately. Calculated summaries are not an ERP reconciliation.</small> : null}</div>;
                      })}
                    </div>
                  ) : (

                  ...
                  transcript.turns.length ? (
                    <div className="call-transcript">
                      {transcript.turns.filter((turn) => !['analysis', 'analysis_candidate'].includes(valueFromRecord(turn, ['role']))).map((turn, index) => {
                        ...
                        return <div className="call-transcript-turn" key={`${index}-${speaker}`} lang={language || undefined} dir="auto"><div className={styles.transcriptMeta}><strong>{isSource ? 'MCP report' : speaker}</strong><span>{isSource ? `Report delivery · ${valueFromRecord(turn, ['delivery_state']) || 'received'}` : language ? languageName(language) : 'Language not reported'}</span></div>{presentation ? <><strong>Prepared spoken answer</strong><p>{presentation}</p><small>This is the prepared presentation, not a recording transcript. Playback may have been interrupted.</small></> : null}{isSource ? <details open={!presentation}><summary>Original MCP response — exact source</summary><pre style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', font: 'inherit' }}>{content}</pre></details> : <p>{content}</p>}{isSource ? <small>Source figures are preserved. Calculated summaries are not an ERP reconciliation.</small> : null}</div>;
                      })}
                    </div>
                  ) : (

    ...
    <Layout>
      ...
      {report && <>
        ...
        <div className="cost-kpi-grid">
          ...
          <article className="cost-kpi"><div className="cost-kpi-icon"><CheckCircle2 size={18} /></div><span>Measurable line-item coverage</span><strong className="cost-kpi-number">{percentage(report.summary.full_cost_coverage)}</strong><small>{report.summary.fully_priced_calls} complete · {report.summary.total_calls - report.summary.fully_priced_calls} partial or unpriced</small></article>
        ...
        {report.summary.full_cost_coverage < 1 && report.summary.total_calls > 0 && <div className="provider-alert cost-coverage-alert" role="note"><CircleAlert size={15} /><span><strong>{percentage(report.summary.full_cost_coverage)} has complete measurable line-item pricing.</strong> Partial calls remain visible and identify the exact missing input—such as custom-worker hosting, a destination-specific Twilio rate, or a legacy OpenAI token split.</span></div>}

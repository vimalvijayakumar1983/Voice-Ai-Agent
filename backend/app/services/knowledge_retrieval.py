...
def _is_query_stop_word(token: str) -> bool:
    # Check the spoken form before singularisation: ``does`` would otherwise
    # become ``doe`` and slip past the list as a substantive topic term.
    return token in _QUERY_STOP_WORDS or _singular(token) in _QUERY_STOP_WORDS


def _query_tokens(value: str) -> set[str]:
    return {
        _singular(token) for token in _base_tokens(value) if not _is_query_stop_word(token)
    }
...
def _postgres_fts_terms(query: str) -> list[str]:
    return sorted(
        token for token in _tokens(query) if not _is_query_stop_word(token) and token.isalnum()
    )[:16]

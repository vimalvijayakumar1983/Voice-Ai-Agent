...
class _GeneratedAgentSpec(BaseModel):
    ...
    provider: Literal["smallest", "sarvam", "elevenlabs", "inworld", "soniox"]
    ...
    providers = sorted(
        {
            ...
            for voice in voices
            if voice.get("provider") in {"smallest", "sarvam", "elevenlabs", "inworld", "soniox"}
            and voice.get("synthesizer_model")
            and not voice.get("unavailability_reason")
        }
    )
...
Use only the supplied provider names, language codes, and exact knowledge-base names."""
    user_payload = {
        ...
        "output_schema": {
            ...
            "provider": "smallest, sarvam, elevenlabs, inworld, or soniox",

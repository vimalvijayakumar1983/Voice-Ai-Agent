...
def mark_knowledge_bindings_live(knowledge_base: KnowledgeBase) -> None:
    """Record that every bound agent reads the current VAV knowledge directly.

    VAV runtimes retrieve approved knowledge on every caller turn, so a source
    change is live as soon as it is indexed locally.  There is no provider
    publish step to wait for.
    """
    now = datetime.now(UTC)
    for binding in knowledge_base.agent_bindings:
        agent = binding.agent
        provider = str(getattr(agent, "voice_provider", "") or "").strip()
        binding.provider = provider or KNOWLEDGE_PROVIDER
